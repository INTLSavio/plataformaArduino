...
class AdminController{
    ...
    async rsorteio(req, res){
        ...
        const users = await sequelize.query("UPDATE batalhas SET equipe1= ''  WHERE id = 14", { type: QueryTypes.SELECT });
    ...
    async sorteio(req, res){
        ...
        try{
            ...
            var users = []
            users = await AdminService.findUsers(modalidade)//trazendo todos os usuarios com mesma modalidade do admin
            var contador = users.length
            console.log('contador:', contador)

            var posAdmin;
            for(let x=0; x<contador; x++){//for para descobrir posição do adm
                
                console.log(users[x].nome, x)
                if(users[x].nome != 'admin' && users[x].nome !='savio'){//se nao atender os nomes do admin
                    
                    
                }else{//caso atenda é um adimin, dai é só remover do array a posição do admin 
                    posAdmin=x;
            ...
            var rodada;
            if(req.body.rodada == 'Rodada 1'){
                rodada = 1
            }
            if(req.body.rodada == 'Rodada 2'){
                rodada = 2
            }
            ...
            console.log(modalidade)
                ...
                if(modalidade=='Regular' && rodada==1){//se a modalidade do update for regular, o id começa em 6
                    console.log('tecnico rodada 1')
                    y=18;
                }
                if(modalidade=='Regular' && rodada==2 || modalidade=='Técnico' && rodada==2){//condições de definição por classificação atendidas
                    if(modalidade=='Regular' && rodada==2){
                        users = await UserService.classificacao(modalidade)
                        contador = users.length
                        y=21;
                        for(i, y; i<=contador; i=i+2, y++){//vai funcionar até que o i valha 6, sendo incrementado em 2
                            if(i<contador){
                                var equipe1 = users[i].equipe
                                var campo = 'equipe1'
                                var id = y+1
                                await AdminService.updateFinal(id, equipe1, campo, modalidade)
                                if(i+1<contador){
                                    var equipe2 = users[i+1].equipe
                                    var campo = 'equipe2'
                                    var id = y+1;
                    ...
                    if(modalidade=='Técnico' && rodada==2){
                        users = await UserService.classificacao(modalidade)
                        contador = users.length
                        y=15;
                        for(i, y; i<=contador; i=i+2, y++){//vai funcionar até que o i valha 6, sendo incrementado em 2
                ...
                if(modalidade=='Técnico' && rodada==1){
                    console.log('tecnico rodada 1')
                    y=12
                }
                for(i, y; i<=contador; i=i+2, y++){//vai funcionar até que o i valha 6, sendo incrementado em 2
